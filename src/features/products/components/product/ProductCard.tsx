...
export function ProductCard({ product }: ProductCardProps) {
	...
	return (
		<article className="group relative flex h-full flex-col justify-between rounded-[2rem] border border-emerald-950/5 bg-white p-4 shadow-md transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
			<div>
				{/* Image Container */}
				<div className="relative h-72 sm:h-80 w-full overflow-hidden rounded-[1.5rem] bg-slate-50 border border-gray-100/60">
					{/* Category Badge */}
					{product.category?.label && (
						<span className="absolute left-3 top-3 z-10 rounded-full bg-white/80 backdrop-blur-md px-3.5 py-1 text-xs font-semibold text-emerald-900 border border-white/25 shadow-sm pointer-events-none">
							{product.category.label}
						</span>
					)}
				...
				<div className="mt-4 px-1">
					...
					<div className="mt-1 flex items-baseline justify-between gap-2">
						<h3 className="text-lg sm:text-xl font-playfair font-semibold leading-tight text-slate-950">
							<Link
								to={APP_ROUTES.PRODUCT(product.id)}
								className="hover:text-emerald-800 transition-colors focus:outline-none focus:underline"
							>
								{product.name}
							</Link>
						</h3>
			...
			<OriginButton
				...
				className={cn(
					"mt-5 w-full h-10 px-4 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-95",
