...
import { useCartSummary } from "@/services/cart/queries";
import {
	useRemoveFromCart,
	useUpdateCartItemQuantity,
} from "@/services/cart/mutations";
...
export const CartPage = () => {
	...
	const { data: summary, isLoading, isFetching, isError } = useCartSummary();

	useDocumentTitle(`${t("cart.title")} | Zamazor`);

	if (isLoading) return <div>Loading summary...</div>;
	if (isError || !summary) return <div>Error loading cart details.</div>;
					...
					<div className="grid gap-8 lg:grid-cols-[1fr_360px] items-start">
						...
						<div className="space-y-4">
							...
							>
								...
								{language === "fr"
									? "Continuer à acheter des formules"
						...
						<div className="space-y-4">
							<div className="bg-white rounded-3xl border border-emerald-900/5 p-5 sm:p-6 shadow-md shadow-emerald-950/5">
								...
								<div className="space-y-3.5 text-sm">
									...
									<div className="flex justify-between text-base font-black text-slate-900 border-t border-slate-100 pt-4 mt-2">
										...
										<span>
											{isFetching ? "calculating.." : `${total.toFixed(2)} MAD`}
										</span>
									</div>
								</div>
							...
							<div className="rounded-2xl border border-emerald-900/5 bg-[#f0f7ec] p-4 text-center">
								<p className="text-[11px] leading-relaxed text-slate-600">
									🌿 Zamazor orders are processed within 24 hours. Subscription
									stacks save an additional 15% on repeat deliveries with total
