...
import { useCreateOrUpdateAddress } from "@/features/addresses/hooks/use-address";
...
export const ProfilePage = () => {
	...
	const [cancelConfirmOrder, setCancelConfirmOrder] = useState<Order | null>(
		null,
	);
	...
	const { data: orderPage, isPending } = useUserOrders(
		{},
		{
	...
	const handleCancelOrder = async () => {
		try {
			...
			await cancelOrderMutation.mutateAsync(cancelConfirmOrder.id);
		} finally {
	...
	return (
		<div className="min-h-screen bg-[#fcfdfa] py-12 px-4 sm:px-6 lg:px-8">
			<div className="mx-auto max-w-5xl">
				{/* Top Welcome Card */}
				<div className="bg-linear-to-r from-emerald-950 to-emerald-900 rounded-[2.5rem] p-6 sm:p-10 text-white shadow-xl shadow-emerald-950/5 mb-8">
					<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
						<div>
							<span className="text-xs font-black uppercase text-lime-300 tracking-widest">
							...
							<p className="text-sm text-emerald-100/75 mt-1">
								{language === "fr"
									? "Gérez vos coordonnées, adresses de livraison et la régularité de vos commandes."
				...
				{activeTab === "profile" ? (
					<div className="grid gap-8 md:grid-cols-3">
						...
						<div className="overflow-hidden rounded-[2rem] border border-emerald-900/8 bg-white shadow-sm h-fit">
							<div className="h-1 w-full bg-linear-to-r from-emerald-500 via-emerald-700 to-lime-400" />
							<div className="p-6">
								<div className="flex items-start gap-4">
									...
									<div className="min-w-0">
										...
										<p className="mt-1 text-sm leading-6 text-slate-500">
											{language === "fr"
												? "Votre profil connecté et l'adresse enregistrée."
												: "Your signed-in profile and saved address."}
										</p>
									</div>
								</div>
					...
					<div className="bg-white rounded-3xl border border-emerald-900/5 p-6 sm:p-8 shadow-xs">
						...
						{isPending || !orderPage ? (
							...
							<div className="text-center py-12">
								...
								<h3 className="text-xl font-playfair text-slate-900">
									{t("profile.emptyOrders")}
								</h3>
								<p className="mt-2 text-sm text-slate-500 max-w-sm mx-auto">
									{language === "fr"
										? "Créez votre routine bien-être et passez votre première commande."
											...
											<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100/70 pb-4.5 mb-4.5">
												<div className="space-y-1">
													<div className="flex items-center gap-2">
														...
														<span
															className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase tracking-wider ${meta.badgeClass}`}
														>
															{meta.label}
														</span>
													</div>
												...
												<div className="text-right">
													...
													<span className="text-lg font-extrabold text-emerald-950">
														{parsePrice(order.total).toFixed(2)} MAD
													</span>
												</div>
											</div>
