...
import { useCart, useCartSummary } from "@/services/cart/queries";
import {
	useCheckout,
	useGetPaymentUrl,
	useVerifyPayment,
} from "@/features/orders/hooks/use-order";
import { useClearCart } from "@/services/cart/mutations";
...
export const CheckoutPage = () => {
	...
	useEffect(() => {
		...
		if (!orderId || !sessionId || !cartSummary || isSuccess) return;
				...
				<div className="mt-6 flex flex-wrap items-center justify-center gap-3">
					...
					>
						<Link to={APP_ROUTES.CHECKOUT}>
							{language === "fr" ? "Revenir au paiement" : "Return to checkout"}
						</Link>
					</Button>
					<Button
						asChild
	...
	const submitPayment = async (data: CheckoutFormValues) => {
		...
		try {
			...
			getPaymentUrlMutation.mutate(response.id);
		} finally {
	...
	if (cartItems?.length === 0 && !isSuccess) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center bg-[#fcfdfa] p-4 text-center">
				...
				>
					<Link to={APP_ROUTES.SHOP}>
						{language === "fr" ? "Parcourir les formules" : "Browse formulas"}
					</Link>
				</Button>
			</div>
		);
			...
			<div className="min-h-screen bg-[#fcfdfa] flex flex-col items-center justify-center p-4">
				<div className="max-w-md w-full bg-white rounded-3xl border border-emerald-900/10 p-6 sm:p-8 text-center shadow-xl shadow-emerald-950/5">
					...
					<p className="text-sm text-slate-500 mt-2">
						{language === "fr"
							? "Votre commande de compléments propres a été passée avec succès. Les détails ont été envoyés à "
							: "Your clean stack order has been successfully placed. We've sent a receipt details and updates to "}
					...
					<div className="my-6 p-4 bg-emerald-50/50 rounded-2xl border border-emerald-900/5 text-left text-sm space-y-2">
						...
						<div className="flex justify-between">
							...
							<span className="font-extrabold text-emerald-900">
								{cartSummary?.total.toFixed(2)} MAD
							</span>
						</div>
					</div>
					...
					<div className="flex items-center gap-2 justify-center text-xs text-emerald-800 font-bold uppercase tracking-wider mb-6">
						...
						<span>
							{language === "fr"
								? "La régularité commence maintenant"
	...
	return (
		<div className="min-h-screen bg-[#fcfdfa] text-slate-900 selection:bg-emerald-100 flex flex-col justify-between">
						...
						>
							...
							<div className="bg-white rounded-3xl border border-emerald-900/5 p-5 sm:p-6 shadow-xs">
								<h2 className="font-playfair text-xl font-bold text-slate-950 mb-4 flex items-center gap-2 border-b border-slate-100 pb-3">
									<span className="size-6 bg-emerald-900 text-white rounded-full flex items-center justify-center text-xs font-bold font-sans">
										1
									</span>
									{t("checkout.step1")}
								</h2>
									...
									{isSubmitting ? (
										...
										<>
											...
											{language === "fr"
												? "Payer avec Stripe"
												: "Pay with Stripe"}{" "}
											&bull; {cartSummary?.total.toFixed(2)} MAD
										</>
									)}
								</OriginButton>
							</div>
							...
							<div className="bg-white rounded-3xl border border-emerald-900/5 p-5 sm:p-6 shadow-md shadow-emerald-950/5">
								...
								<div className="space-y-2.5 text-xs border-t border-slate-100 pt-4">
									<div className="flex justify-between text-slate-500">
										<span>Subtotal</span>
										<span className="font-bold text-slate-900">
											{cartSummary?.subtotal.toFixed(2)} MAD
										</span>
									</div>
										...
										{cartSummary?.shipping == null ? (
											...
											<span className="font-bold text-slate-900">
												{cartSummary.shipping === 0 ? (
													...
													`${cartSummary.shipping.toFixed(2)} MAD`
												)}
											</span>
										)}
									</div>
									...
									<div className="flex justify-between text-sm font-black text-slate-900 border-t border-slate-100 pt-3 mt-2">
										...
										<span>{cartSummary?.total.toFixed(2)} MAD</span>
									</div>
								</div>
							</div>
								...
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-[10px] text-slate-500 leading-normal mt-0.5">
											If your stack doesn't help you feel more consistent within
											30 days, we'll refund you 100%. No questions asked.
										</p>
									</div>
								</div>
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-[10px] text-slate-500 leading-normal mt-0.5">
											All Zamazor deliveries are fully insured. If your canister
											is damaged in transit, we ship a replacement immediately.
										</p>
									</div>
								</div>
							</div>
			...
			<footer className="text-center py-8 text-xs text-slate-400 border-t border-emerald-900/5 mt-12 bg-white font-sans">
				<p>
					&copy; {new Date().getFullYear()} Zamazor Clean Supplements. All
					rights reserved. Secure 256-bit encryption connection.
				</p>
			</footer>
		</div>
	);
