...
export const QuizModal = ({ onClose }: QuizModalProps) => {
	...
	const recommended = useMemo(() => {
		if ((productPage?.items ?? []).length === 0 || !quizFocus) return null;
		...
		return (
			(productPage?.items ?? []).find(
				(p) => p.category.label === map[quizFocus],
			) || (productPage?.items ?? [])[0]
		);
	}, [productPage?.items, quizFocus]);
		...
		>
			<div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-linear-to-br from-emerald-50/95 to-lime-50/60 border border-emerald-900/10 rounded-3xl shadow-2xl p-6 sm:p-10">
				...
				<div className="text-center mb-8">
					...
					<h2 className="mt-2 text-2xl font-playfair font-normal text-slate-950">
				...
				{quizStep === "intro" && (
					<div className="flex flex-col items-center justify-center text-center py-6">
						...
						<p className="text-slate-500 text-sm max-w-sm mt-3 leading-relaxed">
							Answer three quick questions about your health focus, diet, and
							activity level.
						</p>
						<Button
						...
						<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
							{QUIZ_FOCUSES.map((opt) => (
								<button
									key={opt.id}
									onClick={() => {
										setQuizFocus(opt.id as QuizFocus);
										setQuizStep("diet");
									}}
							...
							<div className="flex-1 text-center md:text-left flex flex-col justify-between">
								<div>
									...
									<p className="text-xs text-slate-500 mt-2 leading-relaxed">
										...
										<strong className="text-emerald-950">{quizFocus}</strong>{" "}
										and active schedule, this premium clean formula delivers
										bioavailable nourishment with zero synthetics.
									</p>
								</div>
								<div className="mt-4 flex items-center justify-between flex-wrap gap-2 border-t border-slate-100 pt-3">
									<span className="text-lg font-black text-slate-900">
										{recommended.price} MAD
									</span>
									<div className="flex gap-2 w-full sm:w-auto">
										<Button
											onClick={() => handleAddToCart(recommended.id)}
										...
										<Button
											variant="outline"
											onClick={() => {
												navigate(`/product/${recommended.id}`);
												onClose();
											}}
