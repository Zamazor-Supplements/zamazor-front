	...
	return (
		<div className="flex items-center gap-3 px-2.5 py-2 mb-4 rounded-xl border border-slate-100/50 select-none bg-slate-50/50">
			...
			<div className="flex flex-col overflow-hidden">
				<span className="text-[13px] font-bold leading-none mb-1 text-slate-900 truncate max-w-37.5">
					{current}
				</span>
				<span className="text-[10px] text-slate-500 leading-none font-medium">
					Admin Desk
				</span>
			</div>
		</div>
        ...
        `}
            ...
            `}
				...
				<div className="flex items-center gap-2">
					{item.shortcut && (
						<kbd className="hidden group-hover:inline-flex items-center justify-center h-5 px-1.5 text-[10px] font-medium font-mono text-slate-400 bg-slate-50 border border-slate-200 rounded-lg shadow-xs">
							{item.shortcut}
						</kbd>
					)}
					{item.badge && (
						<span className="flex items-center justify-center min-w-5 h-5 px-1.5 text-[10px] font-bold rounded-full bg-emerald-50 text-emerald-800 border border-emerald-100">
							{item.badge}
						</span>
					)}
					{hasChildren && (
...
export function SidebarNav({
	...
	return (
		<div
			className={`flex flex-col w-65 h-full bg-white text-slate-800 border-r border-slate-200/85 p-4 font-sans ${className}`}
		>
